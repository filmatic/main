...
public class RateMovieServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String customerRating = (String) request.getParameter("movieRating");
            String orderNumber = (String) request.getParameter("movieToRate");
            
            Orders order = filmaticBean.getOrder(orderNumber);
            Movie movie = order.getMovieId();
            
            Integer currentRating = movie.getRating();
            movie.setTimesRated(movie.getTimesRated() + 1);
            
            if (movie.getTimesRated() == 1) {
                currentRating = Integer.parseInt(customerRating);
            } else {
                currentRating += Integer.parseInt(customerRating);
